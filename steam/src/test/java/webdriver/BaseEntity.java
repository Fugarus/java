...
public abstract class BaseEntity {
    ...
    private static List<String> softErrors = new ArrayList<String>();
    ...
    protected void fatal(final String message) {
        ...
        AssertJUnit.assertTrue(formatLogMsg(message), false);
    }
    ...
    public void softAssertEquals(Object exp, Object act, String message){
        if(!exp.equals(act)){
            softErrors.add(String.format("%s. Actual:%s  Ecpected:%s",message, act, exp));
        }
    }

    public static void checkSortErrors(){
        if (softErrors.size() == 0){
            return;
        }
        for (String error : softErrors) {
            errorMessage(error);
        }
        fail();
    }
