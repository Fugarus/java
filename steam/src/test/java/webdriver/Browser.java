...
import static webdriver.BaseEntity.errorMessage;
...
public final class Browser {
	...
	public void back(){
		...
		Logger.getInstance().info("Page was backed.");
	}
	...
	public void switchToLastWindow(){
		...
		int size=getAllWindows().size();
		for(int i=0;i<10;i++,list=getAllWindows()) if(list.size()>size)break;
	...
	public void closeLastWindow(){
		List<String>list=getAllWindows();
		getDriver().switchTo().window(list.get(list.size()-1)).close();
		getDriver().switchTo().window(list.get(0));
	}

	public void clickJsByCoordinates(int x,int y){
		JavascriptExecutor executor =
				(JavascriptExecutor)getDriver();executor.executeScript(String.format("$(document.elementFromPoint(%d,%d)).click();", x,y));
	}

	public String screenVisiblePage(String name) {
		String saveImageDir=DIR_SAVE_IMAGE + name + "." + EXPANSION_IMAGE;
		try {
			File screenFile =
